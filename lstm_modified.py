...
for train, test in kfold.split(values, label_train):
    ...
    def make_model(metrics=METRICS, output_bias=None):
        model=Sequential([LSTM(1000, batch_input_shape=(BATCH_SIZE, TIME_STEPS, x_t.shape[2]), stateful=True, return_sequences=True), Dropout(k), LSTM(1000, batch_input_shape=(BATCH_SIZE, TIME_STEPS, x_t.shape[2]),  stateful=True,  return_sequences=False), Dropout(k), Dense(200,activation='relu'),  Dense(200,activation='relu'), Dense(1,activation='sigmoid')])
