...
for train, test in kfold.split(values, label_train):
    ...
    def make_model(metrics=METRICS, output_bias=None):
        model=Sequential([Conv1D(filters=512, kernel_size=8, activation='relu', input_shape=(n_timesteps, n_features)),
                          Dropout(0.2),Conv1D(filters=512, kernel_size=8, activation='relu'),Dropout(0.2),MaxPooling1D(pool_size=2),Flatten(), Dense(200,activation='relu'), Dense(200,activation='relu'), Dense(1,activation='sigmoid')])
